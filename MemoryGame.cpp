...
private:
 ...
 vector<int> colorPool = {36, 35, 33, 32, 34, 31};
 ...
 void calculateScore() { score = max(0, 1000 - moves * 10 + matches * 150); }
...
public:
 ...
 void setDifficulty(int opt) { gridSize = (opt == 1 ? 2 : 4); }
 ...
 void showHighs() {
  ...
  if (highs.empty()) {
   cout << YELLOW << "No high scores yet.\n\n" << RESET;
  } else {
 ...
 void play(bool timed = false) {
  ...
  while (matches < totalPairs) {
   ...
   if (!validPick(r1,c1)) { msleep(700); continue; }
   int idx1 = r1 * gridSize + c1;
   if (board[idx1].flipped || board[idx1].matched) { msleep(600); continue; }
   ...
   if (!validPick(r2,c2) || (r1 == r2 && c1 == c2)) { board[idx1].flipped = false; msleep(700); continue; }
 ...
 void menu() {
  while (true) {
   clearScreen();
   printHeaderBox();
   cout << BOLD << WHITE << "1) " << RESET << "New Game\n";
   cout << BOLD << WHITE << "2) " << RESET << "Timed Game\n";
